...
package body Radar_Internals is
   ...
   begin
      Antenna_Angle :=
        Antenna_Angle +
        (case Speed is when Slow => 7, when Normal => 17, when Fast => 59,
           when Stopped => 0);
   end Rotate_Antenna;
   ...
   begin
      ...
      if Active_Object_Type = Object_Type_T'Last then
         ...
         Active_Object_Type :=
           Object_Type_T'Val (Object_Type_T'Pos (Active_Object_Type) + 1);
      end if;
   end Next_Object;
   ...
   begin
      return
        (case Current_Walk_Speed is when Stopped => 0.0, when Slow => 4.0,
           when Normal => 8.0, when Fast => 30.0);
   ...
   begin
      Put
        ("T =" & Natural'Image (T) & "  Antenna" &
         Angle_Degrees_T'Image (Antenna_Angle) & "deg." & ASCII.HT & "ETA");
      if Has_E_T_A then
         declare
            E_T_A_H : Integer :=
              Integer (Float'Floor (Time_To_Arrival / 3_600.0));
            E_T_A_M : Integer :=
         ...
         begin
            Put
              (Integer'Image (E_T_A_H) & "h" & Integer'Image (E_T_A_M) & "m" &
      ...
      if Active_Object_Distance < 5.0 and Active_Object_Status = Out_Of_Range
      then
         Active_Object_Status := Tracked;
      end if;

      Put_Line
        (ASCII.HT & Object_Type_T'Image (Active_Object_Type) & " is " &
         Object_Status_T'Image (Active_Object_Status));
      T := T + 1;
   end Time_Step;
   ...
   is
      E_T_A_H : Integer := Integer (Float'Floor (Time_To_Arrival / 3_600.0));
      E_T_A_M : Integer :=
        Integer (Float'Floor (Time_To_Arrival / 60.0)) mod 60;
      E_T_A_S : Integer := Integer (Float'Rounding (Time_To_Arrival)) mod 60;
   begin
      Put_Line
        ("T =" & Natural'Image (T) & "  " & "ETA" & Integer'Image (E_T_A_H) &
         "h" & Integer'Image (E_T_A_M) & "m" & Integer'Image (E_T_A_S) & "s" &
